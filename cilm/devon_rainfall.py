...
fred = pd.read_csv("C:\\Users\\lenovo\\OneDrive\\Documents\\academia\\extremexp\\cilm\\data\\devon_rainfall.csv")
fred.columns = ['DATE',"RAIN"]
fred['DATE'] = pd.to_datetime(fred['DATE'],dayfirst=True)
fred.index = fred.DATE
data = fred.drop(['DATE'],axis=1)
data = pd.Series(data['RAIN'])
cilm1 = lm.CILM(10,data,"RAIN","DATE",0.90)
out = cilm1.sim()
...
data = fred.drop(['DATE'],axis=1)
data = pd.Series(data['RAIN'])
cilm1 = lm.CILM(20,data,"RAIN","DATE",0.95)
out = cilm1.sim()
out.index = pd.to_datetime(out["DATE"])
out = out.drop(["DATE"],axis=1)
classical_case = out[["ACTUAL","NLCI","NUCI"]]
...
data = fred.drop(['DATE'],axis=1)
data = pd.Series(data['RAIN'])
cilm1 = lm.CILM(100,data,"RAIN","DATE",0.99)
out = cilm1.sim()
out.index = pd.to_datetime(out["DATE"])
out = out.drop(["DATE"],axis=1)
classical_case = out[["ACTUAL","NLCI","NUCI"]]
